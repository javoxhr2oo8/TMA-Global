...
const handleAdd = () => addItem(props.product)
const handleMinus = () => updateQuantity(props.product.id, quantity.value - 1)
const handlePlus = () => updateQuantity(props.product.id, quantity.value + 1)
</script>
...
<template>
    <div class="group relative h-full flex flex-col overflow-hidden rounded-[12px] 
        ...
        <!-- Всё внутри NuxtLink кроме кнопок -->
        ...
        <div class="px-[10px] pb-[10px] flex flex-col gap-[10px]">
            ...
            <Button v-if="!inCart" @click="handleAdd"
                class="!p-0 !rounded-xl !bg-green-700 hover:!bg-green-800 !shadow-lg">
                <i class="fa-solid fa-cart-shopping text-white text-sm"></i>
                Savatga +
            </Button>

            <div v-if="inCart" class="flex gap-[10px] items-center">
                <Button @click="handleMinus" class="w-full text-[16px] !p-[3px]">
                    <i class="fa-solid fa-minus"></i>
                </Button>
                <span class="text-[13px] text-white font-semibold">{{ quantity }}</span>
